---
name: gsap-plugins
description: GSAP 插件能力索引 —— 插件注册、ScrollToPlugin、ScrollSmoother（丝滑滚动）、Flip（布局 FLIP 过渡）、Draggable（拖拽）、Inertia（惯性）、Observer、SplitText（文字逐字逐行动画）、ScrambleText、MorphSVG（SVG 形状变换）、DrawSVG、MotionPath、CustomEase / CustomBounce / CustomWiggle。当需要文字逐字进场、布局过渡、SVG 高级动画、拖拽手势、滚动平滑等高级效果时使用。GSAP 3.13+ 所有插件已免费商用。
license: MIT
---
