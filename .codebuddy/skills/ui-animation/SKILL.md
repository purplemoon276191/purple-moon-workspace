...
> **核心定位**：framer-motion 是 React 项目的**默认动画方案**，覆盖 90% 的常规需求。**GSAP 是高级补充**，当 framer-motion 表达能力不够、或需要专业级运动控制时启用。两者**共存不冲突**，模板已同时预装 `framer-motion@^12` 和 `gsap@^3.13`。

### 分流决策表

| 需求场景 | 选择 | 对应 Skill |
|---------|------|-----------|
| 通用入场/退场、滚动 reveal | **framer-motion** (MotionPrimitives) | 本 skill Part 1 |
| 简单 Hover、Tap 交互 | **framer-motion** | 本 skill Part 1 |
| Hero / Landing 创意组件 | **ReactBits** | 本 skill Part 2 |
| **复杂滚动剧情**（pin 钉住、scrub 进度联动、多段触发） | **GSAP** | `gsap-scrolltrigger` |
| **文字逐字/逐行动画**（SplitText）、打字机、文字 morph | **GSAP** | `gsap-plugins`（SplitText） |
| **复杂时间轴编排**（多元素精确编舞、可回放/反向/seek） | **GSAP** | `gsap-timeline` |
| **布局过渡 / FLIP 动画**（元素位置切换+无缝过渡） | **GSAP** | `gsap-plugins`（Flip） |
| **SVG 形状变换 / 路径绘制 / 沿路径运动** | **GSAP** | `gsap-plugins`（MorphSVG / DrawSVG / MotionPath） |
| 拖拽 + 惯性物理 | **GSAP** | `gsap-plugins`（Draggable / Inertia） |
| ScrollSmoother 丝滑滚动 | **GSAP** | `gsap-plugins`（ScrollSmoother） |
| 数字滚动统计 | **ReactBits** (`CountUp` / `Counter`) | 本 skill Part 2 |
| 一般卡片 Hover 微动效 | **framer-motion** | 本 skill Part 1 |

### 升级触发关键词

当用户需求中出现以下信号时，**主动切到 GSAP** 而不是硬用 framer-motion：

- "做个炫酷的产品官网/Landing Page，要有滚动剧情"
- "首屏标题要逐字浮现 / 打散重组"
- "页面滚动到某段时元素要钉住，伴随进度变化"
- "复杂的产品故事页，多段动画要精确配合"
- "SVG 图标要从一个形状 morph 成另一个"
- "需要可以拖拽 + 自然惯性回弹"
- "滚动要丝滑 / 类似 Apple 官网那种"

### GSAP 使用入口

引入 GSAP 时按需触发对应子 skill：

| 触发场景 | 使用 Skill |
|---------|-----------|
| 任何 GSAP 代码起手 | `gsap-react`（useGSAP hook + 清理规范，**React 项目必读**） |
| 滚动驱动 | `gsap-scrolltrigger` |
| 时间轴编排 | `gsap-timeline` |
| 核心补间 API | `gsap-core` |
| 插件能力（SplitText / Flip / MorphSVG / Draggable 等） | `gsap-plugins` |
| 性能优化 | `gsap-performance` |
| 数值映射 / 随机 / 吸附等辅助 | `gsap-utils` |

### 混用最佳实践

GSAP 和 framer-motion **可以同页面共存**：

```tsx
// Hero 区：GSAP 处理复杂滚动剧情 + SplitText 标题
import { useGSAP } from "@gsap/react";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { SplitText } from "gsap/SplitText";

// 通用 Section：framer-motion 处理常规进场
import { FadeIn, Stagger, HoverLift } from "@/components/MotionPrimitives";
```

**分工原则**：

- **首屏 Hero / 重点叙事段** → GSAP 主导
- **常规内容区 / 列表 / 卡片 / 表单** → framer-motion 主导
- **创意装饰组件**（粒子背景、光标特效）→ ReactBits 主导

不要在同一个元素上同时用两套动画库控制 transform，否则会互相覆盖。

### 许可证说明

GSAP 自 2025-04-30 起完全免费商用（含 ScrollTrigger / SplitText / MorphSVG / Flip 等全部插件），Genie 生成的应用使用 GSAP **无需任何额外授权**。详见 <https://gsap.com/licensing/>。
