...
### 配额限流处理（MANDATORY — issue #3156）

`ensure-cloudbase-env.sh` 创建 TCB 环境时，若用户已达会员配额上限，脚本会：
- 以 **退出码 `42`** 结束（约定：`42`=配额限流，`43`=资源隔离）
- 在 stdout 输出哨兵包裹的标准化错误：`##GENIE_TCB_ERROR##{json}##END##`，其中 json 含 `code` 与 `display`

**你（Agent）必须这样处理，不得当普通失败重试：**
1. 检测到脚本退出码为 `42` → 判定为配额限流，**立即停止**该 TCB 相关操作，**不要重试、不要换方式绕过**。
2. 从 stdout 提取 `##GENIE_TCB_ERROR##` 与 `##END##` 之间的 JSON，取其 `display` 字段。
3. **将 `display` 原样呈现给用户**（标题 + 说明 + 升级/管理入口），并告知"云开发环境已达当前会员档位上限，需升级会员或释放已有环境后重试"。
4. 不要继续后续依赖 TCB 的步骤（建表/CRUD 等）。

> `display` 由后端统一产出，直接展示即可，无需你自己拼文案。
