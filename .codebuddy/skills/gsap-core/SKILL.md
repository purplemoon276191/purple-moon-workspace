---
name: gsap-core
description: GSAP 核心 API 动画能力 —— gsap.to() / from() / fromTo()、easing、duration、stagger、defaults、gsap.matchMedia()（响应式 + prefers-reduced-motion）。当需要专业级 JS 动画引擎、为 React 组件做基础补间、SVG/DOM 动画、需要 easing 控制或响应式/无障碍动画时使用。优于 framer-motion 的场景：复杂时间轴编排、需要更细粒度的运动控制、SVG 动画。
license: MIT
---

> **来源**：[greensock/gsap-skills](https://github.com/greensock/gsap-skills)（MIT License, Copyright (c) 2026 GreenSock）。
> 已对 description 中文化以适配 Genie 触发场景，正文保留官方原版以确保技术细节准确性。
