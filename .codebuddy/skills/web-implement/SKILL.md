...
- **Animations**: Use the **ui-animation** skill for all animation needs:
  ...
  - **Advanced (GSAP)**: For **complex scroll choreography**, **text-character/line animation (SplitText)**, **timeline orchestration**, **FLIP layout transitions**, or **SVG morphing** — switch to GSAP. Template pre-ships `gsap@^3.13` (all plugins free since 2025-04-30). Trigger the matching skill: `gsap-react` (always start here for React), `gsap-scrolltrigger`, `gsap-timeline`, `gsap-plugins`. See `ui-animation` skill **Part 5: 何时升级到 GSAP** for the decision table.
